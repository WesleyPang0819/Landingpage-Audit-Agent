...
import { 
  Zap, AlertCircle, CheckCircle2, Lock, LogOut, Shield, User 
} from 'lucide-react';
  ...
  return (
    <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 h-16 md:h-20 flex items-center justify-between px-4 md:px-8 sticky top-0 z-[100]">
      <div className="flex items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => navigate('/')}>
        ...
        <span className="font-black text-lg md:text-xl text-slate-900 tracking-tight hidden xs:block">
          {curText.title.split(' ')[0]}<span className="text-blue-600 ml-1">{curText.title.split(' ').slice(1).join(' ')}</span>
        </span>
      </div>
